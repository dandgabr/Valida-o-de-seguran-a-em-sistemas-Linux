    ...
    def generate_bash_script(cls, result: AssessmentResult) -> str:
        ...
        for comp in active_components:
            ...
            if comp == "sysctl":
                ...
                lines.append("EOF")
                lines.append("    if sshd -t >> \"${LOG_FILE}\" 2>&1; then")
                lines.append("        systemctl reload ssh >> \"${LOG_FILE}\" 2>&1 || systemctl reload sshd >> \"${LOG_FILE}\" 2>&1 || true")
                lines.append("        print_success \"OpenSSH reconfigurado e recarregado.\"")
                lines.append("    fi")
                ...
                lines.append("    if systemctl list-unit-files 2>/dev/null | grep -q \"auditd\"; then")
                lines.append("        systemctl enable --now auditd >> \"${LOG_FILE}\" 2>&1 || true")
                lines.append("        print_success \"Serviço auditd ativado.\"")
                lines.append("    fi")

            elif comp == "aide":
                lines.append("    if command -v aide >/dev/null 2>&1; then")
                lines.append("        if [ ! -f /var/lib/aide/aide.db ] && [ ! -f /var/lib/aide/aide.db.gz ]; then")
                lines.append("            aideinit -y >> \"${LOG_FILE}\" 2>&1 || aideinit >> \"${LOG_FILE}\" 2>&1 || true")
                lines.append("            [ -f /var/lib/aide/aide.db.new ] && cp -f /var/lib/aide/aide.db.new /var/lib/aide/aide.db")
                lines.append("            print_success \"Base AIDE inicializada em /var/lib/aide/aide.db\"")
                lines.append("        fi")
                lines.append("    fi")
